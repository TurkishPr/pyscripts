...
for folder in cls_folders: #loop through each cls_fn/fp folder
    ...
    for fn in fn_list: #loop through each fn
            ...
            with open(det_file, "r") as file:
                for line in file:
                    ...
                    if fn_img_name == gt_jpgName and _IOU>0.5:
                        ...
                        # confs.append([conf, fn_cls, fn_img_name, xmin2, ymin2, xmax2, ymax2])
        ...
        if found_fn:
            ...
            # cv2.rectangle(image,(int(confs[0][3]),int(confs[0][4])),(int(confs[0][5]),int(confs[0][6])),(0,0,255),1)
        ...
        if not found_fn: #if fn is not found in the detection txt files, it is most likely not detected at all. Pretty sure :/
