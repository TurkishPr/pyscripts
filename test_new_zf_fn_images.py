...
prints it onto the image and tells you the det results (x1 y1 x2 y2 conf)
ex: car -> bus .87
...
folder = "Car_fn" #where the fn is in the fpfn_crop folder
fn = 'cm_ger.1760_20181221_063623_n.mp4_00360.jpg_282.0_105.0_908.0_644.0.jpg' #image name
'''*****************************'''
'''*****************************'''
...
if gt_cls == 'pedestrian':
    gt_cls = 'ped'
print("\nGT CLASS : " + gt_cls + "\n")
new_img_path = os.path.join(main_folder,gt_cls+"_conf")
    ...
    with open(det_file, "r") as file:
        for line in file:
            ...
            if fn_img_name == gt_jpgName and _IOU>0.5:
                fnCount+=1
                print("DET Number : {}".format(fnCount))
                print("Found {} ".format(fn_cls))
                print("GT  bbox : {}".format(box_gt))
                print("FN  bbox : {}\n    conf : {:.3f}".format(box_fn,conf))
                confs.append([conf, fn_cls])
                # print(det_file)
                print("IOU : {}".format(_IOU))
                print("DET DATA : {} ".format(line.rsplit('/')[1]))

                #for 통계
                # if gt_cls.lower() == fn_cls.lower(): #conf 나눠먹기된 경우
        ...
        # if found_fn:
        #     file.close()
# for conf_ in sorted(confs, key=lambda t : t[0], reverse=True):
#     print(conf_)
...
if found_fn:
    ...
    image = cv2.imread(os.path.join(os.path.join(fpfn_crop_path,folder),fn)) #open image
    cv2.putText(image, "[{} -> {} {}]".format(gt_cls,'n/a','0.00'),  (70, 20), cv2.FONT_HERSHEY_PLAIN, 1, (48, 48, 255), 2 )
    cv2.imshow("new",image)
    cv2.waitKey(0)
...
#         # cv2.waitKey(0)
#         # print(os.path.join(new_img_path,fn_img_name))
#         cv2.imwrite(os.path.join(new_img_path,fn_img_name[:-4]+"_"+str(missingCnt)+".jpg"),image) #save image to file
